...
| OLX exact-category and correction safety | Real publish requires current exact leaf metadata and rejects semantic contradictions. Preview/confirmation show provider ID and full path. Import/sync preserve provider metadata. Live mismatches create deduplicated human-review recommendations containing separate fail-closed delist and quota-blocked recreate intents; combined approval and provider side effects are refused. Recreate is represented in the quota ledger but has no execution endpoint in this issue. Deletion restores no quota unit, and advert lifetime is not quota availability. | `docs/olx-category-safety.md`; `OlxCategoryGuard.ts`; publish/import/adapter/quota services; migration `021_listing_marketplace_category.sql`; `ListingDetailsPage.tsx` | Guard, publish, import/dedup, approval, quota, adapter and API preview tests; [#169](https://github.com/ylazakovich/hermes-marketdesk/issues/169) |
