...
| OLX exact-category and correction safety | Real publish requires current exact leaf metadata and rejects semantic contradictions. Preview/confirmation show provider ID and full path. Import/sync preserve provider metadata. Live mismatches create human-review recommendations plus separate persistent delist and recreate lifecycle records. Authenticated workspace routes approve and execute each operation independently; combined approval remains refused. Recreate authorizes quota with `mode=recreate` before provider effects and blocks unknown/stale/exhausted state unless its own persisted paid-risk override was approved. Delist records zero restored quota. Atomic claims and terminal replay prevent HTTP retries from repeating provider effects; interrupted executing operations require manual reconciliation. Advert lifetime is not quota availability. | `docs/olx-category-safety.md`; `CategoryCorrectionOperationService.ts`; `CategoryCorrectionOperationRepository.ts`; `OlxCategoryGuard.ts`; migrations `022_listing_marketplace_category.sql`, `023_hermes_event_idempotency.sql`, and `024_category_correction_operations.sql`; authenticated Hermes routes | Category correction lifecycle/order/idempotency/override tests; authenticated route integration test; guard, publish, import, quota and adapter tests; [#169](https://github.com/ylazakovich/hermes-marketdesk/issues/169) |
