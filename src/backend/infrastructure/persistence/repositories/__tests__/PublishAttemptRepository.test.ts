...
describe('PublishAttemptRepository', () => {
  it('atomically creates the first publishing checkpoint', async () => {
    ...
    await expect(repository.begin('operation-1', 'listing-1', 'olx')).resolves.toMatchObject({
      created: true,
      checkpoint: {
    ...
    expect(String(query.mock.calls[0][0])).toContain('ON CONFLICT (operation_id) DO NOTHING');
  });
